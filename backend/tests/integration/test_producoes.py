...
async def test_atualizar_producao_ajusta_estoque_pela_diferenca(client):
    headers = await _login(client)
    caneca, tinta, papel, produto = await _cenario_completo(client, headers)
    ...
    resposta = await client.put(
        ...
        json={"produto_id": produto["id"], "quantidade_produzida": "20"},
        headers=headers,
    )
    assert resposta.status_code == 200
    corpo = resposta.json()
    assert corpo["custo_total"] == "107.00"  # 5.35 por unidade * 20
    assert corpo["valor_total"] == "300.00"

    caneca_apos = (await client.get(f"/api/materiais/{caneca['id']}", headers=headers)).json()
    tinta_apos = (await client.get(f"/api/materiais/{tinta['id']}", headers=headers)).json()
    papel_apos = (await client.get(f"/api/materiais/{papel['id']}", headers=headers)).json()

    assert float(caneca_apos["quantidade_atual"]) == 80.0  # 100 - 1*20
    assert float(tinta_apos["quantidade_atual"]) == 4900.0  # 5000 - 5*20
    assert float(papel_apos["quantidade_atual"]) == 980.0  # 1000 - 1*20


async def test_atualizar_producao_bloqueada_quando_falta_estoque(client):
    headers = await _login(client)
    _, _, _, produto = await _cenario_completo(client, headers)
    ...
    resposta = await client.put(
        f"/api/producoes/{criada['id']}",
        json={"produto_id": produto["id"], "quantidade_produzida": "10000"},
        headers=headers,
    )
    assert resposta.status_code == 422

    caneca_apos = (await client.get(f"/api/materiais/{criada['itens'][0]['material_id']}", headers=headers)).json()
    assert float(caneca_apos["quantidade_atual"]) == 90.0  # unchanged: rollback on failed edit
